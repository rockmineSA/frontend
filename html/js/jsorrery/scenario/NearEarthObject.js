...
define(
	...
	function(ns, common) {
		...
		var qData = getQueryString();
		var selectedId = qData['selectedId'] != undefined ? qData['selectedId'] : "";
		var astTitle = qData['asteroid'] != undefined ? qData['asteroid'] : "RANDOM";

		var myBodies = {
                _aster :{

                        title : getQueryString['asteroid'],
                        mass : 1,
                        radius : 2000,
                        color : '#ffffff',
                        orbit: {
                                epoch : "20160113.0000000",
                                base : {
                                a : 1.45787774640981000000 * ns.AU,
                                e : 0.22267865886036980000,
                                w : 178.79803679622130000000,
                                M : 95.36114806088854000000,
                                i : 10.82860235235547000000,
                                o : 304.32981827813990000000
                                },
                                day : {
                                        M : 360 / 642.95427314470040000000
                                }
                        }
                }};
		...
		var astroPath = 'http://ec2-54-191-195-51.us-west-2.compute.amazonaws.com:8392/api/rockmine/asteroid?limit=25';
		...
		var onLoadErrorAstro = function(jqXHR, textStatus, errorThrown){
			alert('Error loading AstroData');
			console.log(textStatus, errorThrown);
			console.log(jqXHR);
		};
		...
		var loadAstroData = function(response) {

			for(var index in response) {
				var asteroid = response[index];
				bodies[asteroid.id] = _.extend({
				title : asteroid.full_name,
				orbit: {
					epoch : asteroid.epoch_cal,
					base : {
						a : asteroid.a * ns.AU,
						e : asteroid.e,
						w : asteroid.w,
				...
				if(selectedId != undefined && selectedId != "") {
					if(asteroid.id == selectedId) {
						bodies[asteroid.id]['color'] = '#ffffff';
					} else {
						bodies[asteroid.id]['color'] = '#000000';
					}
				} else {
					bodies[asteroid.id]['color'] = '#ffffff';
				}
		...
		var cnf = {
			...
			load : (function(){
				...
				return function(){
					...
					ready = loaded.then(loadAstroData);
