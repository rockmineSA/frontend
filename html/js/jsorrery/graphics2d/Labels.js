...
define(
	...
	function(ns, $, CameraManager, Dimensions) {
		...
		var getEarthPos = function() {
			for(var index in labels){
				var b = labels[index].body;
				if(b.getName() == 'earth'){
		...
		var getPlanetLabelCallback = function(el, body3d){
			...
			return function(camPos, fov){
				...
				earthPos = getEarthPos();		
				if (screenCoords) {
					if(earthPos != undefined)
					distFromEarth = Math.round(body3d.getPosition().distanceTo(earthPos));
					else distFromEarth = "";
el.remove();
el = $('<div class="planetSpot" data-shown="true"><div class="planetLabel">' + body3d.celestial.title +'( '+ distFromEarth + ') </div></div>').appendTo('body');
